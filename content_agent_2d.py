...
class ContentAgent2D(ContentAgent):
    ...
    def analyze_content_2d(self, content: str, perspective: str = None) -> Dict[str, str]:
        """
        分析2D内容，包含视角维度
        
        Args:
            content: 用户输入内容
            perspective: 用户选择的视角（正视角/仰视角），如果为None则自动识别
            
        Returns:
            Dict[str, str]: 包含表情、动作、视角等维度的分析结果
        """
        logger.info(f"开始2D内容分析: {content}")
        ...
        if perspective and perspective in ["正视角", "仰视角"]:
            result["视角"] = perspective
            logger.info(f"使用用户指定的视角: {perspective}")
        else:
            # 自动从内容中识别视角
            result["视角"] = self.analyze_perspective(content)
        
        logger.info(f"2D内容分析结果: {result}")
        return result
    ...
    def process_content_2d(self, content: str, perspective: str = None) -> Dict:
        ...
        # 3. 内容分析（2D特定版本，包含视角）
        try:
            analysis = self.analyze_content_2d(content, perspective)
            return {
                "success": True,
                "compliant": True,
                "reason": "",
