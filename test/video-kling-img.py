...
def _build_payload(model: str, text: str, image_url: str, duration: int, mode: str) -> Dict[str, Any]:
    return {
        "model": model,
        "content": [
            {"type": "text", "text": text},
        ...
        "parameters": {"duration": int(duration), "mode": mode},
    }
...
def main():
    parser = argparse.ArgumentParser(description="Kling 视频生成与状态查询脚本")
    parser.add_argument("--domain", default=DEFAULT_DOMAIN)
    parser.add_argument("--model", default="Kling-V2-5-Turbo")
    parser.add_argument("--text", default="镜头转起来")
    parser.add_argument("--image-url", default="https://maas-task.s3.cn-north-1.jdcloud-oss.com/test-upload/shenyuede/18488982a9ac47af8feddfc85b8ce514.png")
    ...
    if not task_id:
        ...
        payload = _build_payload(args.model, args.text, args.image_url, args.duration, args.mode)
