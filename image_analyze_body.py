...
class ImageAnalyzer:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "你是一个专业的图像分析专家。我给你的是IP整体形象，请忽略IP头像的细节，只关注身体部分，请按照 '手部姿势、腿部姿势、整体姿势、姿势意义、情感偏向' 五个维度对图片进行分析，精简结论。请按照以下格式回答：\n手部姿势：[描述]\n腿部姿势：[描述]\n整体姿势：[描述]\n姿势意义：[描述]\n情感偏向：[描述]"
                    },
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": "请分析这张IP头像图片，按照手部姿势、腿部姿势、整体姿势、姿势意义、情感偏向五个维度进行分析。"},
    ...
    def analyze_folder(self, folder_path: str, output_excel: str = "analysis_results.xlsx") -> None:
        ...
        for i, image_path in enumerate(image_files, 1):
            ...
            row_data = {
                ...
                "手部姿势": analysis_result["手部姿势"],
                "腿部姿势": analysis_result["腿部姿势"],
                "整体姿势": analysis_result["整体姿势"],
                "姿势意义": analysis_result["姿势意义"],
                "情感偏向": analysis_result["情感偏向"],
    ...
    def save_to_excel(self, results: List[Dict], output_file: str) -> None:
        ...
        try:
            ...
            # 确保列的顺序
            columns_order = ["图片名", "手部姿势", "腿部姿势", "整体姿势", "姿势意义", "情感偏向", "图片url地址"]
            df = df[columns_order]
...
def main():
    ...
    # 获取用户输入
    while True:
        folder_path = "D:\project\dongdesign\joy_ip\data\joy_body_noface"
        if folder_path and Path(folder_path).exists():
            break
        print("文件夹路径无效，请重新输入")
    
    # 获取输出文件名
    output_file = "D:\project\dongdesign\joy_ip\data\joy_body_noface.xlsx"
    if not output_file:
